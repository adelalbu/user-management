...
export class AppComponent implements OnInit {
  ...
  apiURL = 'http://77.81.246.11:3000/users';
  ...
  public checkId(): void {
    this.service.checkIfUserExists().subscribe((data: User[]) => {
      ...
      const listOfUsersWithId = data.filter(user => user.id == this.userToBeAdded.id);
  ...
  public addUser() {
    ...
    this.http.post(this.apiURL, this.userToBeAdded).toPromise().then(result => {
      console.log(result);
  ...
  public updateUser() {
    ...
    this.http.put(this.apiURL + '/' + this.userToBeEdited.id, this.userToBeEdited)
  ...
  public deleteUser() {
    ...
    this.http.put(this.apiURL + '/' + this.userToBeEdited.id, this.userToBeEdited).toPromise().then(result => {
      console.log(result);
      window.location.reload();
    });
